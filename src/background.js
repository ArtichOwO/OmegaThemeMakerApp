      ...
      submenu: [
        {
          ...
          click: async () => {
            shell.openExternal(
              "https://github.com/ArtichOwO/OmegaThemeMakerApp/"
            );
          },
        },
        {
...
function createTouchBar() {
  const { TouchBarLabel, TouchBarButton, TouchBarSpacer, TouchBarColorPicker } =
    TouchBar;
...
function importFile() {
  ...
  dialog.showOpenDialog(openProps).then((file) => {
    if (file === undefined) {
      win.webContents.send(
        "addLnToStatus",
        '<span style="color: #FF0000;">No file selected</span>'
      );
      return;
    }
    ...
    fs.readFile(file.filePaths[0], "utf-8", (err, data) => {
      if (err) {
        win.webContents.send(
          "addLnToStatus",
          '<span style="color: #FF0000;">An error ocurred reading the file :' +
            err.message +
            "</span>"
        );
        return;
