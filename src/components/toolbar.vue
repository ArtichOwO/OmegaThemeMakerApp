<template>
  <div id="toolBar" class="grayContainer noselect">
    <img
      src="../assets/toolBar/launchbutton.png"
      alt="toolBar"
      id="runButton"
      @click="run"
    />
    <label for="importButton">
      <img
        src="../assets/toolBar/importfile.png"
        id="imageImportButton"
        alt="Import"
      />
    </label>
    <input type="file" name="Import" id="importButton" @input="importFile" />
    <img
      src="../assets/toolBar/savebutton.png"
      alt="Save"
      id="saveButton"
      @click="save"
    />
    <img
      src="../assets/toolBar/screenshotbutton.png"
      alt="ScrnShot"
      @click="screenshot"
    />
    <img
      src="../assets/toolBar/fullscreen.png"
      alt="FullScrn"
      @click="fullscreen"
    />
  </div>
...
export default {
    ...
    importFile() {
      ...
      if (file.type.match(textType)) {
        ...
        reader.onload = function () {
          ...
          _this.$store.commit(
            "addLnToStatus",
            "Imported " + _this.$store.state.theme.name + " from " + file.name
          );
        };
        ...
        this.$store.commit(
          "addLnToStatus",
          '<span style="color: #FF0000;">File not supported!</span>'
        );
        return false;
      }
    },
  ...
  mounted() {
    window.ipcRenderer.on("fullscreen", () => {
      this.fullscreen();
    });
    ...
    window.ipcRenderer.on("screenshot", () => {
      this.screenshot();
    });
  },
};
